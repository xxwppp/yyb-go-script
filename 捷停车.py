...
#
# 【青龙环境变量说明】
# YYB_GO 必填：wxcode服务地址，多地址用换行或&分隔
...
# ===================== 配置项 =====================
# 从环境变量 YYB_GO 读取wxcode服务地址，换行或&分隔
SERVERS = []
...
if len(SERVERS) == 0:
    ...
    print("青龙环境变量YYB_GO填写示例（每个地址用换行或&分隔）：")
...
def build_proxy_transport(proxy_info: Dict[str, Any]) -> Optional[AsyncProxyTransport]:
    ...
    try:
        if PROXY_TYPE == "socks5":
            ...
            proxy_url = f"https://{username}:{password}@{host}:{port}" if username and password else f"http://{host}:{port}"
