...
环境变量：YYB_GO（格式 server地址@openid，多账号用 & 或换行分隔）
...
# YYB_GO 解析（格式 server地址@openid，多账号用 & 或换行分隔）
YYB_GO_ENTRIES: List[str] = []
_env_yyb_go = os.environ.get("YYB_GO", "")
if _env_yyb_go:
    YYB_GO_ENTRIES = [line.strip() for line in _env_yyb_go.replace("&", "\n").splitlines() if line.strip()]
...
def build_yyb_auth_header() -> Dict[str, str]:
    """YYB 协议统一认证：优先 YYB_TOKEN，否则 YYB_USER/YYB_PASS 做 Basic auth"""
    token = os.environ.get("YYB_TOKEN", "")
    if token:
        return {"Authorization": f"Bearer {token}"}
    user = os.environ.get("YYB_USER", "")
    pwd = os.environ.get("YYB_PASS", "")
    if user and pwd:
        import base64
...
class YybCodeProvider:
    ...
    def list_accounts(self) -> List[CodeAccount]:
        ...
        if not self.server:
            log("未配置 YYB_GO，且 YYB_REFS 为空")
            return []

        url = f"{self.server}/accounts"
        try:
            response = self.session.get(url, headers=build_yyb_auth_header(), timeout=REQUEST_TIMEOUT)
    ...
    def get_code(self, account: CodeAccount) -> Optional[str]:
        if not self.server:
            log("未配置 YYB_GO，无法请求 /wxapp/getCode")
            return None
        ...
        headers.update(build_yyb_auth_header())
        ...
        for attempt in range(1, RETRY_TIMES + 1):
            try:
                ...
                if code:
                    log(f"{account.label} 获取 wx code 成功")
                    return code
                log(f"{account.label} 获取 code 为空 resp={str(data)[:160]}")
            except Exception as exc:
                log(f"{account.label} 获取 code 异常：{str(exc)[:120]}")
...
def auto_fetch_accounts() -> List[BusinessAccount]:
    ...
    total = len(YYB_GO_ENTRIES)

    for index, entry in enumerate(YYB_GO_ENTRIES, 1):
        if "@" not in entry:
            log(f"❌ YYB_GO 格式应为 地址@微信账号标识，当前值：{entry}")
            continue
        server, ref = entry.split("@", 1)
        provider = YybCodeProvider(server.strip())
        code_account = CodeAccount(label=yyb_display(entry), ref=ref.strip())
        log(f"处理账号[{index}/{total}] {yyb_display(entry)}")
    ...
    log(f"业务账号获取成功 {len(business_accounts)} / {total}")
...
def main() -> int:
    ...
    if not accounts:
        log("未获取到业务账号。请检查 YYB_GO 或 MANUAL_ACCESS_TOKENS。")
        return 1
