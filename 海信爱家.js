...
说明：openid 首次自动登录会通过 YYB Go 的 /wxapp/getPhoneNumber 获取手机号授权 code。
*/
...
const wechat = {
  ...
  async getuserinfo(entry) {
    const { server, openid } = parseYybGoEntry(entry);
    const { data } = await axios.post(`https://${server}/wxapp/operateWxData`, {
    ...
    if (!encryptedData || !iv) throw new Error('YYB_GO operateWxData(getUserInfo) 未返回 encryptedData/iv: ' + JSON.stringify(data));
...
class Task {
  ...
  async getPhoneNumberData() {
    ...
    const { data } = await wechat.getphonenumber(this.openid)
