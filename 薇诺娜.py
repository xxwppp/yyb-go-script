...
def refresh_account_token(server_entry: str):
    ...
    print(f"🔄 正在刷新账号 {yyb_display(server_entry)} 的Token...")
    ...
    if new_token:
        ...
        print(f"✅ 账号 {yyb_display(server_entry)} Token刷新成功")
        return new_token
    else:
        print(f"❌ 账号 {yyb_display(server_entry)} Token刷新失败")
        return None
...
def main():
    ...
    for index, server_entry in enumerate(SERVERS, 1):
        _, wxid = parse_yyb_go_entry(server_entry)
        remark = f"账号{index}({yyb_display(server_entry)})"
